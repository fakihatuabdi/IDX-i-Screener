...
**Scalping** sejak 2026-09-20 tidak lagi bagian dari update harian jam 19:00 WIB - diganti scan **real-time tiap 5 menit selama jam bursa** (09:00-15:50 WIB, Senin-Jumat), plus **satu run persiapan pra-bursa jam 19:00 WIB di hari sebelumnya** (bukan pagi-pagi mepet jam buka - kebanyakan sekuritas sudah bisa terima order submit dari ~jam 20:00 malam sebelumnya, jadi prep-nya disiapkan sejak sore/malam) memakai kode dan parameter screening yang persis sama - bukan logika lama yang dihidupkan lagi. Jadwalnya ikut pola `daily-update.yml` (hari 0-4/Minggu-Kamis, karena Jumat malam tidak perlu prep untuk Sabtu yang bursa tutup). Karena data intraday hari berikutnya belum ada, VWAP/EMA/RSI7 pada run ini otomatis memakai nilai real terakhir dari penutupan sesi hari itu (bukan dikarang). Ditandai `session_phase:"pre_market"` di data (dihitung dari jam WIB riil saat run, bukan dari cron mana yang memicunya) dan dilabeli jelas di dashboard ("Prep pra-bursa") supaya tidak disalahartikan sebagai data live intraday. memakai data intraday Pluang (`lib/scalping.mjs`, `scripts/scalping-scan.mjs`, `.github/workflows/scalping-scan.yml`): VWAP sesi dari candle 5 menit riil, Micro EMA3/5/9, RSI(7), Order Book Dynamics (rasio bid/ask riil dari order book), Tape Reading/HAKA (dari running-trades riil, tag BUY/SELL asli), dan volume breakout terarah. Verdict-nya pakai matriks hitung sinyal generik persis dokumen referensi "Buku Putih Logika Aplikasi Trading" (Bullish&ge;4 & Bearish=0 &rarr; Strong Buy, dst) - deliberately BEDA dari skor berbobot Swing/Investment di atas, karena ini modul baru tanpa perilaku live lama yang perlu dijaga kompatibel (lihat `ROADMAP.md` §1.4/§3.3 untuk keputusan verdict matrix Swing/Investment yang masih menunggu approval terpisah). Universe: 250 saham teraktif (di-screening murah tiap siklus lewat Multi Quote batch), 30 saham paling bergerak di-deep-scan penuh (order book + tape reading) - desain dua tingkat ini dijelaskan lengkap di `ROADMAP.md` §3.4 termasuk perhitungan kuota API-nya.
...
Untuk verdict **Sell/Strong Sell dan Hold** (semua strategi), tidak ada Entry/Max Buy sama sekali - hanya Target 1, Target 2, dan Stop Loss (Hold = bukan taruhan arah), semuanya tetap dihitung dari ATR riil saham itu, bukan persentase tetap. Setiap kartu saham juga menampilkan **Harga Terakhir (Close)** terpisah, supaya selalu jelas bedanya harga pasar riil sekarang vs zona entry/Max Buy yang direkomendasikan.

**Kartu Sell/Strong Sell (2026-09-21)** dirombak jadi **panduan keluar** untuk yang sudah memegang saham, bukan setup short-sell yang bisa langsung dieksekusi - mayoritas investor ritel IDX tidak punya akses mudah ke short selling. Angka di baliknya tidak berubah (tetap level ATR/struktural riil yang sama), cuma label dan framing-nya: Target 1/2 jadi **"Proyeksi Turun 1/2"** (seberapa jauh saham ini secara riil masih bisa turun), Stop Loss jadi **"Level Waspada"** (kalau harga naik balik melewati level ini, thesis bearish/jual mulai diragukan - sinyal untuk meninjau ulang, bukan trigger stop-loss short). Kartu Scalping Sell juga tidak lagi menampilkan sel "Entry" terpisah.
