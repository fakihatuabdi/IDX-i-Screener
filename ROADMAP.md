...
| Entry: `(Sentuh SMA50 OR Fib 0.618) AND Bandarmologi` | ✅ **Selesai (2026-09-20, diperbaiki)** - anchor SMA50 ATAU Fibonacci 0.618 riil (dipilih yang lebih dekat ke harga); Bandarmologi (`broker_buy_concentration>=60%`) tadinya cuma poin skor tambahan, sekarang jadi **syarat AND wajib** persis dokumen - zona pullback (SMA50/Fib) cuma dipakai kalau Bandarmologi hari itu juga terpenuhi, kalau tidak jatuh ke fallback zona sempit di bawah close | `computeDirectionalLevels` di `lib/pipeline.mjs` (`bandarmologiOk`) |
