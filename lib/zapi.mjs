 ...
 * Every other /v1/finance:idx/* endpoint here that filters by one stock (corporateActions)
 * takes a bare ticker under the key "code" - this previously used "symbol" instead, which the
 * endpoint silently ignored, always returning the same default dataset regardless of which
 * stock was actually requested (proven live: 5 different requested tickers came back with
 * byte-identical per-broker rows). Renamed to match the endpoint's real, working convention.
 */
export function brokerSummary({ symbol, length = 200 }) {
  return callZapi("/v1/finance:idx/broker-summary", { code: symbol, length }).then((d) => d.data);
}
