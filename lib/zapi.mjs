...
/**
 * Real top-10 buyer/seller brokers for ONE stock on a given day, from Pluang (via Zapi) -
 ...
 * average execution price for that specific stock and broker. `date` must be an actual
 * trading day (YYYY-MM-DD) - it isn't defaulted to "today" here because "today" is empty on
 * a non-trading day.
 */
export function pluangBrokerSummary({ code, date }) {
  return callZapi("/v1/finance:pluang/broker-summary", { code, startDate: date, endDate: date }).then((d) => d.data);
}
