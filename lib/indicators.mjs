...
/**
 * Wilder's ATR(14): average true range in real Rupiah terms - how much this specific stock
 * actually moves per session. Used to size entry/stop-loss/target off each stock's own real
 * volatility instead of one flat percentage applied to every ticker alike.
 */
export function atr(highs, lows, closes, period) {
  ...
  if (n > period) {
    let sm = 0;
    for (let i = 1; i <= period; i++) sm += tr[i] || 0;
    out[period] = sm / period;
    for (let i = period + 1; i < n; i++) {
...
/**
 * Most recent real swing/pivot low (Buku Putih §3A "Previous_Swing_Low") - a genuine local
 * minimum in the real daily lows: a session whose low sits at or below every real low in the
 * `lookback` sessions on both sides of it, not a rolling-window-split approximation. Only
 * pivots with enough real sessions after them to actually confirm they were a local minimum
 * are considered, so the most recent usable pivot is always at least `lookback` sessions old -
 * scans backward from there and returns the first (most recent) genuine one found, or null if
 * none exists in the given series.
 */
export function findSwingLow(lows, lookback = 3) {
  const n = lows.length;
  for (let i = n - 1 - lookback; i >= lookback; i--) {
    let isPivot = true;
    for (let k = 1; k <= lookback && isPivot; k++) {
      if (lows[i] > lows[i - k] || lows[i] > lows[i + k]) isPivot = false;
    }
...
export function computeIndicators(candles) {
  ...
  const atr14Series = atr(highs, lows, closes, 14);
  ...
  // Real swing/pivot low (Buku Putih §3A "Previous_Swing_Low") - a genuine local minimum in
  // the real daily lows, not the prior-50-day window's single lowest point. Computed over
  // history up to (never including) today's own bar - see findSwingLow below.
  const swingLow = round1(findSwingLow(lows.slice(0, last)));
  ...
  return {
    ...
    // Yesterday's real MACD line/signal - lets a caller detect a genuine "crosses above" EVENT
    // (was below/equal yesterday, is above today) instead of just today's static position,
    // same distinction as the reference methodology's own wording (Buku Putih §3A).
    macdLinePrev: round2(macdDefault.macdLine[last - 1]), macdSignalPrev: round2(macdDefault.signalLine[last - 1]),
    atr14: round2(atr14Series[last]),
    ...
    swingLow, lastLow: round1(lastLow),
