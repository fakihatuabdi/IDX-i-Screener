...
export function computeIndicators(candles) {
  ...
  // Last-20-day series for each MA/EMA, aligned index-for-index with candles20 below, so
  // the chart can draw a real line that follows price shape over time - not just a flat
  // reference line at today's value.
  const series20 = (arr) => arr.slice(-20).map(round1);
  ...
  return {
    ...
    ema13Series: series20(ema13), ema21Series: series20(ema21),
    sma20Series: series20(sma20), sma50Series: series20(sma50), sma200Series: series20(sma200),
    rvol: round2(rvol),
    pattern, verdict,
