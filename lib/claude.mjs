...
Kembalikan HANYA JSON valid dengan struktur persis ini (tanpa markdown, tanpa penjelasan lain):
{
  ...
  "catalysts": { "TICKER": "1-2 kalimat catalyst spesifik untuk saham ini berdasarkan verdict dan indikator TEKNIKAL-nya saja (jangan sebut angka fundamental di sini - ini dipakai untuk strategi Scalping/Swing yang murni teknikal). Kalau lastClose sudah menembus prior20High/prior50High (breakout resistance) atau jatuh di bawah prior20Low/prior50Low (breakdown support) yang diberikan, WAJIB sebutkan level breakout/breakdown riil itu - ini bukti validitas analisis, jangan dilewatkan kalau datanya ada", ... satu entri per ticker di daftar kandidat },
  "investment_catalysts": { "TICKER": "2-3 kalimat khusus untuk strategi Investment: WAJIB sebutkan angka fundamental riil yang tersedia untuk saham ini (PER, PBV, Dividend Yield, Payout Ratio, DER, ROE - hanya yang ada datanya, skip yang null) sebagai DASAR UTAMA analisis, bukan pelengkap, lalu simpulkan apakah valuasi & kualitas fundamentalnya mendukung atau melemahkan tesis investasi jangka panjang. Kombinasikan dengan verdict teknikalnya (rating TradingView) DAN, kalau datanya ada, sebutkan juga posisi harga terhadap resistance/support 52 minggu (yearHigh/yearLow) - misalnya breakout ke level tertinggi 52 minggu sebagai konfirmasi momentum jangka panjang, atau posisi dekat yearLow sebagai sinyal waspada", ... satu entri per ticker di daftar kandidat }
}`;

  return callClaudeForJson(client, prompt, 4500);
}
