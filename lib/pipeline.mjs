...
import { verdictFromCounts } from "./scalping.mjs";
...
// Calendar quarter-end (month, day) per period label - an approximation (assumes a standard
// calendar-year fiscal year, true for the large majority of IDX issuers) used only to pick
// which real historical closing price corresponds to each real quarterly BVPS below.
const QUARTER_END = { Q1: [3, 31], Q2: [6, 30], Q3: [9, 30], Q4: [12, 31] };

/**
 * Investment's real historical average PBV (Buku Putih §4B Target 2: "PBV_Current >
 * PBV_Historical_Avg x 2") - computed from REAL past data only: each past quarter's real BVPS
 * (from the fundamentals scraper) divided into the real closing price on the nearest real
 * trading day on/before that quarter's end (from the daily candles already fetched for this
 * ticker's indicators - no extra API call). Never interpolated or estimated - a quarter is
 * just skipped if no real candle exists on/before its end date (freshly-listed stock, or a
 * quarter older than the ~300 daily candles already on hand).
 */
function historicalPbvAvg(candles, quarters) {
  if (!candles?.length || !quarters?.length) return null;
  const pbvs = [];
  for (const q of quarters) {
    if (!(q.bvps > 0) || !QUARTER_END[q.period]) continue;
    const [month, day] = QUARTER_END[q.period];
    const quarterEnd = Date.UTC(q.year, month - 1, day);
    let closest = null;
    for (const bar of candles) {
      const t = new Date(bar.date).getTime();
      if (t <= quarterEnd && (!closest || t > closest.t)) closest = { t, close: bar.close };
    }
    if (closest) pbvs.push(closest.close / q.bvps);
  }
  return pbvs.length ? pbvs.reduce((a, b) => a + b, 0) / pbvs.length : null;
}
...
function computeDirectionalLevels(strategy, verdict, c) {
  ...
  // Swing's stop-loss additionally references a real detected swing-low PIVOT (Buku Putih §3B
  // "Close < Previous_Swing_Low" - see lib/indicators.mjs findSwingLow) - a separate, more
  // local level from structLow/prior50Low above (which stays as-is for the Fibonacci range and
  // the pullback-zone floor check below - a proper Fibonacci measurement needs that wider real
  // swing span, not a tiny recent pivot).
  const swingLowPivot = strategy === "swing" ? c.swingLow : null;
  // Swing-specific real Fibonacci levels (Buku Putih §3B) - from the same real 50-day swing
  // high/low already used for structLow/structHigh above, no new data needed. 0.618
  // retracement doubles as an alternate pullback-entry anchor alongside SMA50; the 1.618
  // extension is Target 2's real structural objective instead of a generic reward:risk
  // projection - both only computed (and only used below) when strategy is "swing".
  let fib618 = null, fibExt1618 = null;
  if (strategy === "swing" && structHigh != null && structLow != null) {
    const range = structHigh - structLow;
    fib618 = structHigh - range * 0.618;
    fibExt1618 = structLow + range * 1.618;
  }
  ...
  // Swing's own pullback anchor is "touches SMA(50) OR Fibonacci(0.618)" (Buku Putih §3B) -
  // use whichever real level sits closer to today's price (the shallower, more immediate real
  // support), not SMA50 alone.
  let anchor = c[cfg.supportKey];
  if (strategy === "swing" && fib618 != null && fib618 < lastClose) {
    anchor = anchor != null ? Math.max(anchor, fib618) : fib618;
  }
  // Swing's literal entry rule is "(...SMA50 OR Fib0.618...) AND (Bandarmologi==TRUE)" (Buku
  // Putih §3B) - a real, separate AND condition on top of the OR between the two anchor
  // levels above, not just one more additive scoring signal. Only gates the pullback branch
  // below ("Buy on Weakness" - the specific archetype this rule describes); the
  // breakout/anticipatory-breakout archetypes are this codebase's own addition, not covered
  // by this literal doc rule, so they're untouched.
  const bandarmologiOk = c.broker_net_buy_5d === true;
  ...
  if (alreadyBrokenOut) {
    ...
    const pullbackAllowed = strategy !== "swing" || bandarmologiOk;
    if (pullbackAllowed && anchor != null && anchor < lastClose && anchor >= floor) {
      entryLow = anchor;
      entryHigh = lastClose;
    } else {
      entryHigh = lastClose;
  ...
  if (swingLowPivot != null && swingLowPivot < stopLoss && stopLoss - swingLowPivot <= stopDistance) {
    stopLoss = swingLowPivot * 0.995;
  } else if (structLow != null && structLow < stopLoss && stopLoss - structLow <= stopDistance) {
    stopLoss = structLow * 0.995;
  }
  const risk = entryHigh - stopLoss;
  // Target 1: Swing's own real "Nearest Resistance Level" (Buku Putih §3B) when the stock
  // genuinely hasn't broken out past it yet (a real level still ahead of price) - the usual
  // reward:risk projection otherwise (already broken out - there's no nearer known resistance
  // left to name - or for Scalping/Investment, which don't specify this literal rule).
  let target1;
  if (strategy === "swing" && !alreadyBrokenOut && structHigh != null && structHigh > entryHigh) {
    target1 = structHigh;
  } else {
    target1 = entryHigh + cfg.rr1 * risk;
  }
  // Target 2: Swing's own real Fibonacci Extension(1.618) (Buku Putih §3B) when it's a genuine
  // further objective beyond Target 1 - the usual extended reward:risk/52-week-high projection
  // otherwise.
  let target2;
  if (strategy === "swing" && fibExt1618 != null && fibExt1618 > target1) {
    target2 = fibExt1618;
  } else {
    target2 = extendTarget(cfg, target1, risk, false, c.yearHigh);
  }
 ...
 * shares, total_equity/shares - scrapers/idx-fundamentals/, see fundamentals_historical above)
 * when that ticker's been scraped; otherwise derived from the real PER/PBV we always have
 ...
 */
function computeGrahamFairValue(c) {
  const lastClose = c.lastClose;
  ...
  if (fairValue != null && (fairValue < lastClose * 0.1 || fairValue > lastClose * 10)) fairValue = null;
  return fairValue;
}

// Disclosed, fixed DCF rate assumptions - not fetched "live" data (no source publishes a daily
// Indonesia equity cost-of-capital feed this pipeline could pull), but real, cited, published
// reference figures rather than arbitrary guesses: ~11% cost of equity approximates Indonesia's
// market cost of equity per Damodaran's (NYU Stern) published country risk premium framework
// (BI policy rate ~6-7% + an equity risk premium of several points) - the SAME grounding
// documented for the ROE bearish threshold discussed in ROADMAP.md §3.3. ~8% cost of
// debt approximates a typical IDX corporate borrowing rate (risk-free + a modest credit
// spread). 22% is Indonesia's real, published statutory corporate tax rate. 3% terminal growth
// is a conservative long-run nominal-growth proxy, kept well under the discount rate.
const DCF_COST_OF_EQUITY = 0.11;
const DCF_COST_OF_DEBT = 0.08;
const DCF_TAX_RATE = 0.22;
const DCF_TERMINAL_GROWTH = 0.03;
const DCF_YEARS = 5;

/**
 * A real, disclosed-assumption DCF (Discounted Cash Flow) fair value per share - added
 * alongside the Graham Number above (not in place of it) per explicit request. Every INPUT
 * that can be real is real: TTM Free Cash Flow (sum of the last 4 real quarters - see
 * ttm_fcf above), the growth rate projected from this company's own real historical net
 * income growth (capped to a sane range so one anomalous quarter can't be extrapolated into
 * an absurd valuation), the discount rate's capital-structure WEIGHTS from this company's own
 * real DER. Only the discount/terminal-growth RATES themselves are disclosed fixed
 * assumptions (see DCF_COST_OF_EQUITY etc. above) - no live per-company cost-of-capital feed
 * exists to fetch these from, and inventing a fake "real-time" source for them would be worse
 * than a clearly-labeled, cited assumption. Returns null (never a fabricated number) whenever
 * the real inputs don't support a meaningful estimate - negative/missing TTM FCF, a discount
 * rate that wouldn't even exceed the terminal growth rate, missing share count, or a result
 * wildly out of line with the real traded price.
 */
function computeDcfFairValue(c) {
  const lastClose = c.lastClose;
  const f = c.fundamentals || {};
  const hist = c.fundamentals_historical;
  if (!hist || hist.currency === "USD") return null; // same currency guard as computeGrahamFairValue
  const ttmFcf = hist.ttm_fcf;
  const shares = hist.outstanding_shares;
  if (!(ttmFcf > 0) || !(shares > 0)) return null;

  const der = f.debt_to_equity != null && f.debt_to_equity >= 0 ? f.debt_to_equity / 100 : 0; // real DER (e.g. 45 -> 0.45x)
  const weightEquity = 1 / (1 + der);
  const weightDebt = der / (1 + der);
  const discountRate = weightEquity * DCF_COST_OF_EQUITY + weightDebt * DCF_COST_OF_DEBT * (1 - DCF_TAX_RATE);
  if (discountRate <= DCF_TERMINAL_GROWTH) return null; // Gordon Growth is undefined/explosive otherwise

  // Real historical net income growth as the projection basis (a real proxy for cash flow
  // growth when a direct multi-quarter FCF growth series isn't available) - capped to a sane
  // range so one anomalous quarter (seen in practice: >800% swings for some IDX miners) can't
  // be extrapolated five years forward into a meaningless number.
  const rawGrowth = hist.net_income_growth_yoy != null ? hist.net_income_growth_yoy / 100 : 0.05;
  const growth = Math.max(-0.15, Math.min(0.2, rawGrowth));
  ...
  const terminalValue = (fcf * (1 + DCF_TERMINAL_GROWTH)) / (discountRate - DCF_TERMINAL_GROWTH);
  const pvTerminal = terminalValue / Math.pow(1 + discountRate, DCF_YEARS);
  // Simplification, disclosed: treats the projected FCF stream as attributable to equity
  // as-is (no separate net-debt bridge from enterprise to equity value) - this codebase's
  // scraper data doesn't carry a clean "net cash/debt" line, so subtracting one would mean
  // inventing it rather than using something genuinely observed.
  const equityValue = pvSum + pvTerminal;
  let dcfFairValue = equityValue / shares;

  if (dcfFairValue < lastClose * 0.1 || dcfFairValue > lastClose * 10) return null; // same plausibility guard as Graham Number
  return dcfFairValue;
}

/**
 * Combined Fair Value: the average of Graham Number and DCF when BOTH are real and
 * computable, falling back to whichever one is available otherwise (the same graceful-
 * degradation pattern used everywhere else in this codebase - never blocks on one method
 * failing when a real independent one still works).
 */
function computeCombinedFairValue(c) {
  const graham = computeGrahamFairValue(c);
  const dcf = computeDcfFairValue(c);
  if (graham != null && dcf != null) return { fairValue: (graham + dcf) / 2, graham, dcf };
  return { fairValue: graham ?? dcf, graham, dcf };
}

/**
 * Margin of Safety - how far below Fair Value Max Buy sits. Redesigned (2026-09-20, at the
 * user's request) to be driven entirely by real, NAMED value-investing/asset-pricing
 * literature rather than an unsourced baseline nudged by whichever technical signals happened
 * to be handy - each factor below cites its specific source:
 *
 * - Base 30%: Benjamin Graham's own guidance in "The Intelligent Investor" (Ch. 20, "Margin of
 *   Safety as the Central Concept of Investment") - a meaningful discount of "no less than a
 *   third" between price and appraised value, rounded to a clean baseline.
 * - Leverage (real DER): Graham's own defensive-investor balance-sheet criteria - heavier real
 *   debt load is real financial risk, needs a bigger cushion; a genuinely conservative real
 *   balance sheet can accept a smaller one.
 * - Profitability vs cost of equity (real ROE): Damodaran's excess-return framework (Aswath
 *   Damodaran, NYU Stern) - the same ~10% Indonesia cost-of-equity grounding already used for
 *   investmentLiteralEntryVerdict's ROE gate elsewhere in this file. Real ROE well above it
 *   signals real, demonstrated value creation (smaller margin needed); below it, the business
 *   isn't clearly compensating equity holders for their risk (bigger margin needed).
 * - Company size (real cap_tier): the small-cap risk premium - a real, published finding in
 *   asset-pricing literature (Fama & French, "The Cross-Section of Expected Stock Returns",
 *   1992 - the SMB "Small Minus Big" factor) that smaller companies carry real additional
 *   systematic risk, warranting a bigger margin; large, established companies can accept a
 *   smaller one.
 * - Earnings predictability (real revenue/net income growth trend): Seth Klarman's "Margin of
 *   Safety" (1991) - the central thesis that the margin required scales inversely with how
 *   PREDICTABLE the business's real results are; a company with both real revenue and real
 *   earnings genuinely shrinking together is less predictable (bigger margin), one growing
 *   both together is more predictable (smaller margin).
 *
 * Deliberately no longer includes technical/momentum signals (weekly ADX trend, RVOL, broker
 * concentration, corporate actions) that were mixed into this calculation before - those are
 * real signals too, but they're timing/momentum concepts, not margin-of-safety concepts from
 * the value-investing literature this function is now grounded in; kept out so every factor
 * here has a specific, named source. Clamped to [15%, 45%] - Graham's own "no less than a
 * third" floor loosened only slightly for the highest-quality real combination of factors,
 * never below a still-meaningful real discount.
 */
function computeMarginOfSafety(c) {
  const f = c.fundamentals || {};
  const hist = c.fundamentals_historical;
  let marginPct = 0.3;
  if (f.debt_to_equity != null) {
    if (f.debt_to_equity > 200) marginPct += 0.1;
    else if (f.debt_to_equity > 100) marginPct += 0.05;
    else if (f.debt_to_equity <= 50) marginPct -= 0.03;
  }
  if (f.roe != null) {
    if (f.roe >= 20) marginPct -= 0.05;
    else if (f.roe < 10) marginPct += 0.05;
  }
  if (c.cap_tier === "Small Cap") marginPct += 0.05;
  else if (c.cap_tier === "Big Cap") marginPct -= 0.03;
  ...
  return Math.max(0.15, Math.min(0.45, marginPct));
}
...
function computeInvestmentLevels(verdict, c) {
  ...
  // valuation rather than where the chart happens to sit. Fair Value here is the average of
  // Graham Number and DCF (computeGrahamFairValue/computeDcfFairValue above) when both are
  // real and computable, falling back to whichever one is - never fabricated, see those
  // functions' own comments for exactly which inputs are real vs disclosed assumptions.
  const f = c.fundamentals || {};
  const hist = c.fundamentals_historical;
  const { fairValue } = computeCombinedFairValue(c);

  let maxBuy, marginPct = null;
  if (fairValue != null) {
    marginPct = computeMarginOfSafety(c);
  ...
  // Target 1: "Price >= Fair_Value" (Buku Putih §4B - jual sebagian saat harga mencapai nilai
  // wajarnya) - literally the real combined Fair Value (Graham Number averaged with DCF, see
  // computeCombinedFairValue above) when it's available. It should always sit above Max Buy
  // (Max Buy = Fair Value minus a positive margin of safety, so Fair Value > Max Buy by
  // construction) - the usual reward:risk projection is only a fallback for the rare case
  // Fair Value itself couldn't be computed at all (Max Buy already fell back to lastClose).
  const target1 = fairValue != null && fairValue > maxBuy ? fairValue : maxBuy + cfg.rr1 * risk;
  // Target 2: "PBV_Current > PBV_Historical_Avg x 2" (Buku Putih §4B - jual seluruhnya saat
  // market terlalu euforia/overvalued), converted to a real price level: 2 x this ticker's own
  // real historical average PBV (see historicalPbvAvg above - from real past closes divided by
  // real quarterly BVPS, never estimated) x its current real BVPS. Falls back to the usual
  // extended reward:risk projection when there isn't enough real quarterly history to compute
  // a historical average (freshly-scraped or freshly-listed ticker), or when the computed
  // level doesn't even clear Target 1 (a sanity floor, same spirit as the Graham Number
  // plausibility check above).
  let target2 = extendTarget(cfg, target1, risk, false, null);
  if (hist && hist.pbv_historical_avg > 0 && hist.bvps > 0) {
    const pbvTarget = 2 * hist.pbv_historical_avg * hist.bvps;
    if (pbvTarget > target1) target2 = pbvTarget;
  }
  ...
  return {
    ...
    target2: Math.round(target2),
    stop_loss: Math.round(stopLoss),
    // Real margin-of-safety % actually used to discount Fair Value into Max Buy (see
    // computeMarginOfSafety above) - null when Max Buy fell back to lastClose (no real Fair
    // Value could be computed at all), shown on the card so it's never a hidden number.
    margin_of_safety_pct: marginPct != null ? Math.round(marginPct * 1000) / 10 : null,
  };
}
...
export async function buildDashboard({ fundamentalsHistory } = {}) {
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      // Swing's own literal Bandarmologi rule (Buku Putih §3A: "Net_Buy_Volume >
      // Net_Sell_Volume (last 5 days)") - a real, separate check from broker_buy_concentration
      // above (that one stays as-is for Investment's margin-of-safety nudge, a different real
      // metric this codebase already relies on). `net: false` asks for real gross lots so
      // buyers/sellers can be summed and compared directly - approximated from the top-10
      // buy/sell brokers Pluang actually returns (the real data available), same real
      // limitation as broker_buy_concentration above, not the whole market's true volume.
      let broker_net_buy_5d = null;
      if (sessionDate) {
        try {
          const start = new Date(sessionDate);
          start.setUTCDate(start.getUTCDate() - 7); // covers ~5 real trading days including a weekend
          const startDate = start.toISOString().slice(0, 10);
          const bs5d = await withRetry(() => zapi.pluangBrokerSummary({ code: s.ticker, date: sessionDate, startDate, net: false }));
          const buyLots5d = (bs5d.buyers || []).reduce((a, b) => a + (b.lots || 0), 0);
          const sellLots5d = (bs5d.sellers || []).reduce((a, b) => a + (b.lots || 0), 0);
          if (buyLots5d > 0 || sellLots5d > 0) broker_net_buy_5d = buyLots5d > sellLots5d;
        } catch (e) {
          console.error(`pluangBrokerSummary 5-day range unavailable for ${s.ticker}, Bandarmologi gate skipped:`, e.message);
        }
      }
      ...
      // prev_eps_growth_yoy (the quarter immediately before .latest) lets investmentVerdict's
      // forced-Sell check below verify the reference document's own literal Stop Loss rule -
      // "EPS_Growth_YoY < 0 for 2 Quarters" - against two real consecutive quarters.
      const prevQuarter = fhEntry?.quarters && fhEntry.quarters.length >= 2 ? fhEntry.quarters[fhEntry.quarters.length - 2] : null;
      // pbv_historical_avg (see historicalPbvAvg above) reuses the daily `candles` already
      // fetched for this ticker's indicators - no extra API call.
      const fundamentals_historical = fhEntry?.latest
        ? {
            ...fhEntry.latest,
            peg: fhEntry.peg ?? null,
            prev_eps_growth_yoy: prevQuarter?.eps_growth_yoy ?? null,
            pbv_historical_avg: round2(historicalPbvAvg(candles, fhEntry.quarters)),
            // Trailing-twelve-month FCF for the DCF fair value below (computeGrahamAndDcfFairValue)
            // - sums the real last up-to-4 quarters' free_cash_flow (never estimated); null if
            // fewer than 4 real quarters are on hand (Yahoo's own ~5-6 quarter limit already
            // documented elsewhere) rather than annualizing a single quarter into a misleading figure.
            ttm_fcf: (() => {
              const last4 = fhEntry.quarters.slice(-4);
              return last4.length === 4 && last4.every((q) => q.free_cash_flow != null)
                ? round2(last4.reduce((a, q) => a + q.free_cash_flow, 0))
                : null;
            })(),
          }
        : null;
      return { ...s, ...ind, tv_verdict, cap_tier: capTier(s.marketCap), fundamentals, fundamentals_historical, broker_buy_concentration, broker_net_buy_5d };
    } catch (e) {
  ...
  function swingScore(c) {
    ...
    // MACD_Line "crosses above" Signal_Line (Buku Putih §3A) is a real EVENT (was
    // below/equal yesterday, is above today), not just today's static position - fixed from
    // an earlier state-only check, same category of fix as Scalping's VWAP cross (ROADMAP.md
    // §1.3). Yesterday's real MACD values already come from lib/indicators.mjs, no new fetch.
    const macdCrossUp = c.macdLine != null && c.macdSignal != null && c.macdLinePrev != null && c.macdSignalPrev != null
      && c.macdLine > c.macdSignal && c.macdLinePrev <= c.macdSignalPrev;
    const macdCrossDown = c.macdLine != null && c.macdSignal != null && c.macdLinePrev != null && c.macdSignalPrev != null
      && c.macdLine < c.macdSignal && c.macdLinePrev >= c.macdSignalPrev;
    // Buku Putih §3A qualifies the MACD golden cross itself: "Golden Cross di bawah atau dekat
    // angka 0" - an early-stage cross (trend just turning) is worth more than one that's
    // already well above zero (trend already extended). Kept as a BONUS on top of the
    // cross itself (never subtracted from it) so this only adds conviction for the
    // higher-quality case, rather than making the plain cross fire less often than before.
    // "Near zero" is scaled to this stock's own price (0.5% of last close), since MACD's raw
    // Rupiah value has no fixed meaning across tickers at very different price levels.
    const macdEarlyBull = macdCrossUp && c.macdLine <= c.lastClose * 0.005;
    // Net_Buy_Volume > Net_Sell_Volume over the real last ~5 trading days (Buku Putih §3A) -
    // see broker_net_buy_5d in lib/pipeline.mjs's fetchIndicatorsFor. A real, separate signal
    // from broker_buy_concentration (single-day top-3, still used for Investment elsewhere).
    const brokerAccumulating = c.broker_net_buy_5d === true;
    const brokerDistributing = c.broker_net_buy_5d === false;
    ...
    // Price Structure: Current_Low > Previous_Swing_Low (Buku Putih §3A) - a real detected
    // pivot low (lib/indicators.mjs findSwingLow), not the broader HH/HL pattern classification
    // used elsewhere (c.pattern, kept separately for the card's own display text).
    const priceStructureBull = c.lastLow != null && c.swingLow != null && c.lastLow > c.swingLow;
    ...
    if (macdCrossUp) bull += 1;
    if (macdEarlyBull) bull += 1;
    if (macdCrossDown) bear += 1;
    if (brokerAccumulating) bull += 2;
    if (brokerDistributing) bear += 2;
    if (fibZone) bull += 1;
    if (priceStructureBull) bull += 1;
    return { bull, bear, maxBull: 14 };
  }
  ...
  function investmentVerdict(c) {
    ...
    // "Jual paksa" - the reference document's OWN literal Investment Stop Loss rule
    // ("Spesifikasi Algorithmic Trading & ML.pdf" §2: EPS_Growth_YoY<0 for 2 Quarters OR
    // DER>2.0 -> dijual paksa) forces at least a Sell regardless of how the weighted score
    // above reads - real, severe fundamental deterioration overrides a still-positive
    // technical/valuation score, exactly as the doc specifies. Banks excluded from the DER
    // side for the same reason investmentLiteralEntryVerdict's DER gate excludes them below.
    const hist = c.fundamentals_historical;
    const f = c.fundamentals || {};
    const isBank = (c.sector || "").toLowerCase().includes("bank");
    const epsBreakdown = hist && hist.eps_growth_yoy != null && hist.eps_growth_yoy < 0 && hist.prev_eps_growth_yoy != null && hist.prev_eps_growth_yoy < 0;
    const derBreakdown = !isBank && f.debt_to_equity != null && f.debt_to_equity > 200;
    if (epsBreakdown || derBreakdown) return score <= -4 ? "Strong Sell" : "Sell";
  ...
  // ---- Verdict Matrix "Varian B" - Buku Putih Logika Aplikasi Trading.pdf §1's own generic,
  // unweighted Bullish_Count/Bearish_Count matrix, computed IN PARALLEL for Swing and
  // Investment alongside the weighted verdicts above (ROADMAP.md §3.3/§1.4) - exposed as
  // `verdict_b` on each recommendation item, but never used for the real `verdict`, ranking,
  // Entry/Target/Stop-Loss, or Max Buy. The plan (documented in ROADMAP.md) is to compare the
  // two against real outcomes via trade_labels for a few weeks before deciding whether to
  // ever make one of them the live verdict - swapping it outright today would change what
  // gets recommended without any real evidence yet on which approach performs better.

  // [2026-09-20] The Investment module's bull/bear-count matrix (formerly investmentSignals +
  // verdictFromCounts) has been superseded by investmentLiteralEntryVerdict below, which
  // implements Buku Putih §4B's actual literal Entry rule (all 6 gates AND price-discount)
  // instead of the generic §1 Bullish_Count matrix - a more faithful comparison for this
  // module. Its bearish-threshold literature citations (Damodaran ROE, Graham/Brealey-Myers
  // DER>2.0, Graham's own PER x PBV>22.5, Peter Lynch's PEG>2) are preserved in ROADMAP.md
  // §3.3 rather than duplicated here as dead code; the DER and EPS-growth ones remain live in
  // investmentVerdict's own forced-Sell override above.

  /**
   * Investment's OWN literal ENTRY rule (Buku Putih §4B): "(Semua Parameter Fundamental ==
   * TRUE) AND (Price < Fair_Value x 0.7)" - a strict, all-6-gates-simultaneously test, much
   * stricter than both the live weighted verdict (investmentVerdict) AND the generic
   * Bullish_Count matrix used for Swing's verdict_b. Computed only for the "Alt" comparison
   * badge, never the live verdict - literally requiring every one of these 6 real fundamental
   * gates to hold at once is rare enough in real markets that using it live risks leaving
   * Investment's recommendations empty most days (a real, disclosed trade-off - decided with
   * the user 2026-09-20 to keep it informational only). Fair_Value uses computeCombinedFairValue
   * (Graham Number averaged with DCF) rather than DCF alone, per the same 2026-09-20 decision.
   */
  function investmentLiteralEntryVerdict(c) {
    const f = c.fundamentals || {};
    const hist = c.fundamentals_historical;
    const isBank = (c.sector || "").toLowerCase().includes("bank");
    const gates = [
      f.roe != null && f.roe >= 15,
      isBank || (f.debt_to_equity != null && f.debt_to_equity <= 100),
      f.pe_ttm != null && f.pe_ttm > 0 && f.pe_ttm < 15 && f.pb_ratio != null && f.pb_ratio > 0 && f.pb_ratio < 1.5,
      !!(hist && hist.eps_growth_yoy != null && hist.eps_growth_yoy >= 10),
      f.dividend_yield != null && f.dividend_yield >= 5,
      !!(hist && hist.peg != null && hist.peg > 0 && hist.peg <= 1),
    ];
    const allGatesTrue = gates.every(Boolean);
    const { fairValue } = computeCombinedFairValue(c);
    const priceDiscounted = fairValue != null && c.lastClose < fairValue * 0.7;
    return allGatesTrue && priceDiscounted ? "Buy" : "Hold";
  }
  ...
  function buildStrategyList(strategy) {
    ...
    // "Alt" comparison verdicts - only for Swing (generic Bullish_Count matrix) and Investment
    // (literal all-6-gates Entry rule, see investmentLiteralEntryVerdict) - computed once per
    // strategy here rather than inside the per-ticker map for clarity.
    const verdictBFor =
      strategy === "swing"
        ? (c) => { const { bull, bear } = swingScore(c); return verdictFromCounts(bull, bear); }
        : strategy === "investment"
        ? investmentLiteralEntryVerdict
        : null;

    const items = withIndicators.map((c) => {
      const verdict = verdictFor(c);
      const verdictB = verdictBFor ? verdictBFor(c) : null;
      const band =
        strategy === "investment"
          ? computeInvestmentLevels(verdict, c)
      ...
      return {
        ...
        cap_tier: c.cap_tier,
        verdict,
        // Experimental, informational only - Buku Putih's own unweighted verdict matrix run in
        // parallel (ROADMAP.md §3.3). Never used for ranking/Entry/Target/Stop-Loss/Max Buy -
        // only shown as a small "Alt" note on the card when it actually disagrees with the
        // real verdict above, so it's visible for comparison without being mistaken for it.
        verdict_b: verdictB,
  ...
  return {
    ...
    // Real company names for that same universe (already on hand from the screener call above,
    // no extra fetch) - scalping-scan.mjs never calls a fundamentals/profile endpoint itself
    // (it's a fast, frequent, technicals-only scan), so this is its only source for a real name
    // to show on each card.
    scalping_universe_names: Object.fromEntries(screenerItems.slice(0, MOST_ACTIVE_SCAN_COUNT).map((s) => [s.ticker, s.name])),
  };
}
