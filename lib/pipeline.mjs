...
export async function buildDashboard() {
  // ---- 1. Bulk data (Zapi Pro tier: cover the whole exchange, not just a slice) - each
  // wrapped in withRetry like every other call in this file, so a momentary upstream scraper
  // hiccup (real, seen in practice: a transient 503 "Temporarily unavailable" from Zapi's own
  // idx-foreign-flow scraper) gets retried with backoff instead of failing the entire run on
  // the very first bulk fetch.
  const [screenerItems, ffPage0, ffPage1, ffPage2, ffPage3, ihsgIndex, ihsgDaily6m] = await Promise.all([
    ...
    withRetry(() => zapi.idxIndexSummary()),
