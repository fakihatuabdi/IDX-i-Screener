...
export async function buildDashboard() {
  ...
  const [screenerItems, ihsgDaily6m] = await Promise.all([
    withRetry(() => zapi.screener({ count: 300, sortBy: "volume", sortOrder: "desc" })),
  ...
  try {
    ...
    console.error("idxForeignFlow unavailable after retries, falling back to price-momentum ranking instead of net foreign flow:", e.message);
  }
  ...
  // ---- 3. Merge screener + foreign flow, rank candidates (deterministic, no AI). When
  // foreign-flow is down (see above), net_value has no real source at all - candidates are
  // ranked by real price momentum (changePercent) from the screener instead, which is a
  // different and less complete signal, but still genuine data, never fabricated. net_value
  // stays null throughout this fallback path so nothing downstream can mistake it for a real
  // foreign-flow number.
  const screenerByTicker = Object.fromEntries(screenerItems.map((s) => [s.ticker, s]));
  let merged;
  if (foreignFlowAvailable) {
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      return { ...s, ...ind, tv_verdict, cap_tier: capTier(s.marketCap), fundamentals, broker_buy_concentration };
    } catch (e) {
  ...
  const holdPool = merged
    ...
    .sort((a, b) => (foreignFlowAvailable ? Math.abs(a.net_value) - Math.abs(b.net_value) : Math.abs(a.changePercent) - Math.abs(b.changePercent)));
  ...
  const narrative = await writeNarrative({
    ...
    candidates: withIndicators.map((c) => ({
      ...
      // Omitted entirely (not sent as null) when foreign-flow was unavailable this run, so
      // the narrative never references a "net asing" figure that doesn't actually exist today.
      ...(foreignFlowAvailable ? { net_value: c.net_value } : {}),
      rsi14: c.rsi14,
      pattern: c.pattern,
  ...
  function buildStrategyList(strategy) {
    ...
    const technicalFor = (c) =>
      strategy === "scalping"
        ? `EMA9 ${c.ema9} | EMA21 ${c.ema21} | RVOL ${c.rvol}x | RSI(7) ${c.rsi7} | ADX ${c.adx14 ?? "N/A"} | MACD ${macdLabel(c.macdFastLine, c.macdFastSignal)} | Broker Top3 ${c.broker_buy_concentration ?? "N/A"}% | Resistance 20D ${c.prior20High} | Support 20D ${c.prior20Low}`
        : strategy === "investment"
        ? `SMA50 ${c.sma50} | SMA200 ${c.sma200} | RSI(14) Mingguan ${c.weeklyRsi14 ?? "N/A"} | ADX Mingguan ${c.weeklyAdx14 ?? "N/A"} | Rating TradingView: ${c.tv_verdict || "N/A"} | Resistance 52W ${c.yearHigh ?? "N/A"} | Support 52W ${c.yearLow ?? "N/A"} | ROE ${c.fundamentals?.roe ?? "N/A"}% | DER ${c.fundamentals?.debt_to_equity ?? "N/A"}`
        : `SMA20 ${c.sma20} | SMA50 ${c.sma50} | RSI(14) ${c.rsi14} | ADX ${c.adx14 ?? "N/A"} | MACD ${macdLabel(c.macdLine, c.macdSignal)} | Broker Top3 ${c.broker_buy_concentration ?? "N/A"}% | Pola: ${c.pattern} | Resistance 50D ${c.prior50High} | Support 50D ${c.prior50Low}`;
    ...
    const items = withIndicators.map((c) => {
      ...
      const catalyst =
        strategy === "investment"
          ...
          : narrative.catalysts[c.ticker] || `Verdict ${verdict} berdasarkan ${strategy === "scalping" ? "EMA9/21, RVOL, RSI(7), dan konfirmasi ADX/MACD/OBV" : "SMA20/50, RSI(14), ADX/MACD, dan pola HH/HL"}.`;
      return {
        ticker: c.ticker,
        name: c.name,
        cap_tier: c.cap_tier,
  ...
  return {
    ...
    broker_flow: {
      // Real IDX net-foreign-flow data was unavailable this run (see foreignFlowAvailable
      // above) - these stay null/empty rather than a fabricated or stale-looking number, and
      // the frontend uses this flag to hide the panel honestly instead of showing "Rp0".
      foreign_flow_available: foreignFlowAvailable,
      net_foreign_total: foreignFlowAvailable ? merged.reduce((a, b) => a + b.net_value, 0) : null,
      top_net_buy: foreignFlowAvailable ? buySide.slice(0, 5).map((x) => ({ ticker: x.ticker, net_value: x.net_value })) : [],
      top_net_sell: foreignFlowAvailable ? sellSide.slice(0, 5).map((x) => ({ ticker: x.ticker, net_value: x.net_value })) : [],
    ...
    watchouts: sellPicks.slice(0, 3).map((s) => ({
      ticker: s.ticker,
      name: s.name,
      reason: foreignFlowAvailable ? "Net sell asing terbesar, verdict teknikal Sell." : "Performa harga terlemah di shortlist, verdict teknikal Sell.",
    })),
