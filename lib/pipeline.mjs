...
// close: each strategy anchors its entry to the support/resistance level that matches its
// own timeframe - Scalping reads EMA9 (its fastest support), Swing reads SMA20, Investment
// reads SMA50 - the same anchors each strategy's own indicator set already trades off of
// (see technicalFor below). The anchor is only used when it's realistically reachable: bounded
// by that stock's own ATR(14), so a deep, unlikely-to-fill support never gets used as-is, and
// a Buy entry never sits above today's real close (you can't "wait for a pullback" above the
// current price). Stop-loss is sized off that same real ATR risk, then nudged just past a
// genuine nearby swing low/high (matched to the strategy's horizon) when that real level sits
// tighter than the ATR stop - otherwise ordinary noise at that level would trigger the stop
// before the level itself is even tested. Target is entry +/- a reward:risk multiple of that
// same real risk distance, never an arbitrary fixed percentage.
const RISK_CONFIG = {
  scalping: { supportKey: "ema9", stopAtrMult: 1.0, rr: 1.5, maxAtrMult: 0.6, structKey: "prior20" },
  swing: { supportKey: "sma20", stopAtrMult: 1.5, rr: 2.0, maxAtrMult: 1.2, structKey: "prior50" },
  investment: { supportKey: "sma50", stopAtrMult: 2.0, rr: 2.5, maxAtrMult: 2.5, structKey: "year" },
};
...
function computePriceLevels(strategy, verdict, c) {
  ...
  if (verdict === "Hold") {
    // Hold isn't a directional bet, so no support/resistance projection applies - just a
    // tight, symmetric band around today's real close.
    return { entry: Math.round(lastClose), target: Math.round(lastClose * 1.02), stop_loss: Math.round(lastClose * 0.98) };
  }
  ...
  let entry;
  if (isSell) {
    // Sell: entry is a realistic bounce INTO resistance (the price you'd actually act at),
    // capped by ATR so it never drifts unrealistically far above today's close.
    const ceiling = lastClose + maxDistance;
    entry = anchor != null && anchor > lastClose && anchor <= ceiling ? anchor : lastClose;
  } else {
    // Buy: entry is a realistic pullback DOWN to real support, capped by ATR so a support
    // that's too deep to realistically fill isn't used as-is.
    const floor = lastClose - maxDistance;
    entry = anchor != null && anchor < lastClose && anchor >= floor ? anchor : lastClose;
  }

  const stopDistance = cfg.stopAtrMult * atr;
  let stopLoss = isSell ? entry + stopDistance : entry - stopDistance;

  const structLow = c[cfg.structKey + "Low"];
  const structHigh = c[cfg.structKey + "High"];
  if (!isSell && structLow != null && structLow < stopLoss && stopLoss - structLow <= stopDistance) {
    stopLoss = structLow * 0.995;
  }
  if (isSell && structHigh != null && structHigh > stopLoss && structHigh - stopLoss <= stopDistance) {
    stopLoss = structHigh * 1.005;
  }

  const risk = Math.abs(entry - stopLoss);
  const target = isSell ? entry - cfg.rr * risk : entry + cfg.rr * risk;

  return { entry: Math.round(entry), target: Math.round(target), stop_loss: Math.round(stopLoss) };
}
...
export async function buildDashboard() {
  ...
  function buildStrategyList(strategy) {
    ...
    const items = withIndicators.map((c) => {
      ...
      const band = computePriceLevels(strategy, verdict, c);
      const catalyst =
        strategy === "investment"
