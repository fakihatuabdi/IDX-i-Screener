...
export async function buildDashboard() {
  ...
  if (sessionDate) {
    try {
      const foreignList = await withRetry(() => zapi.pluangBrokers({ type: "FOREIGN" }));
      foreignBrokerCodes = new Set((foreignList.items || []).map((b) => b.code));
    } catch (e) {
      console.error("pluangBrokers(FOREIGN) unavailable - foreign-flow proxy fallback won't be usable if needed:", e.message);
    }
  ...
  // 250 stocks rather than the whole exchange, and with no broker firm names available (only
  // codes) since Pluang's per-stock endpoint doesn't carry that field.
  ...
  try {
    ...
    if (brokerScanByTicker.size > 0) {
      ...
      const top10 = [...agg.entries()]
        ...
        .slice(0, 10)
        .map(([code, v]) => ({ broker: code, name: null, volume: v.volume, value: v.value, frequency: null }));
  ...
  // no connection to our own Buy/Sell recommendations. Reuses the Pluang scan fetched above
  // (no extra calls) - a broker whose real top pick fell outside those 250 stocks simply
  // gets no pick shown, never guessed.
  ...
  return {
    ...
    watchouts: sellPicks.slice(0, 3).map((s) => ({
      ...
      reason:
        netValueSource === "idx"
          ? "Net sell asing terbesar, verdict teknikal Sell."
          : netValueSource === "pluang_proxy"
          ? "Estimasi net sell broker asing terbesar (proxy), verdict teknikal Sell."
          : "Performa harga terlemah di shortlist, verdict teknikal Sell.",
    })),
