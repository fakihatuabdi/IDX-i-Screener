...
function computeDirectionalLevels(strategy, verdict, c) {
  ...
  const bandarmologiOk = c.broker_buy_concentration != null && c.broker_buy_concentration >= 60;
...
export async function buildDashboard({ fundamentalsHistory } = {}) {
  ...
  function swingScore(c) {
    ...
    // existing macdBullish above (never subtracted from it) so this only adds conviction for
    // the higher-quality case, rather than making the plain cross fire less often than before.
    ...
    const macdEarlyBull = macdBullish && c.macdLine <= c.lastClose * 0.005;
    const brokerAccumulating = c.broker_buy_concentration != null && c.broker_buy_concentration >= 60;
