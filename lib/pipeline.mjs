  ...
  function buildStrategyList(strategy) {
    ...
    const items = withIndicators.map((c) => {
      ...
      return {
        ...
        // ~1 month grouped into 3-day bars (real OHLCV, just aggregated - denser and more
        // readable than one bar per day); Investment shows the last ~3 months grouped into
        // real weekly bars. MA/EMA lines are resampled the same way so they stay aligned
        // point-for-point with their chart's own candles.
        candles: strategy === "investment" ? c.candlesWeekly : c.candles3d,
        fundamentals: c.fundamentals,
        _sortStrength: strengthScore(verdict),
        _sortFundamental: strategy === "investment" ? fundamentalScore(c.fundamentals) : 0,
        ma_lines:
          strategy === "scalping"
            ? { ema13: c.chart3dLines.ema13, ema21: c.chart3dLines.ema21 }
            : strategy === "swing"
            ? { sma20: c.chart3dLines.sma20, sma50: c.chart3dLines.sma50 }
