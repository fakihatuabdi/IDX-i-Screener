...
export async function buildDashboard() {
  ...
  // ---- 5b. Each active broker's own top buy pick (real, per-stock data - via Pluang, a
  // genuinely different provider from the market-wide broker-summary above). For each of
  // today's top 5 net-buy candidates, fetch the real top-10 BUYER brokers for that specific
  // stock; a broker who appears as a top buyer in more than one of these 5 stocks gets
  // credited with whichever one had the larger real transaction value. A broker from the
  // market-wide list above who wasn't a top-10 buyer in any of these 5 stocks simply gets no
  // top pick here - that's a real absence, never guessed.
  const topPickMap = new Map(); // broker code -> { ticker, value, avg_price }
  if (sessionDate) {
    const topBuyPicks = buyPicks.slice(0, 5);
