...
function computeInvestmentLevels(verdict, c, hasUpcomingCorporateAction) {
  ...
  // (22.5 = Benjamin Graham's own cap of PER<=15 times PBV<=1.5). EPS/BVPS aren't fetched
  // directly but are real numbers derived from the real PER/PBV we do have (EPS = price/PER,
  // BVPS = price/PBV) - never fabricated, and only computed when the company is actually
  // profitable with positive book equity (a Graham Number is meaningless, or literally
  // imaginary, otherwise).
  const f = c.fundamentals || {};
  let fairValue = null;
  if (f.pe_ttm != null && f.pe_ttm > 0 && f.pb_ratio != null && f.pb_ratio > 0) {
