...
export async function buildDashboard({ fundamentalsHistory } = {}) {
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      // prev_eps_growth_yoy (the quarter immediately before .latest) lets investmentSignals
      // below check the reference document's own literal Stop Loss rule - "EPS_Growth_YoY < 0
      // for 2 Quarters" - against two real consecutive quarters, not just the latest one.
  ...
  function investmentVerdict(c) {
    ...
    // side for the same reason the entry gate excludes them (investmentSignals above).
    const hist = c.fundamentals_historical;
    const f = c.fundamentals || {};
  ...
  /**
   * The Investment module's own fundamental gates, straight from Buku Putih §4A for the
   * bullish/"attractive" side of each metric. The reference table itself never defines a
   * bearish side for Investment (unlike Scalping/Swing, which get explicit bull AND bear
   * conditions) - each bearish threshold below is instead grounded in a specific, named,
   * real piece of investing literature/methodology rather than an arbitrary guess:
   *
   * - ROE < 10%: Damodaran's excess-return framework (Aswath Damodaran, NYU Stern) - a company
   *   only creates shareholder value when ROE exceeds its cost of equity; ~10% approximates a
   *   reasonable Indonesia-market cost of equity (BI policy rate ~6-7% + a several-point equity
   *   risk premium per Damodaran's own published country risk premium tables). Below it, the
   *   business is arguably not compensating equity holders for their risk - a real, sourced
   *   line, not just "half of the bullish 15% gate".
   * - DER > 2.0 (200%): the exact number the reference document's OWN Investment Stop Loss
   *   rule uses ("Spesifikasi Algorithmic Trading & ML.pdf" §2 - dijual paksa kalau DER>2.0) -
   *   also a widely-cited generic high-leverage threshold in corporate finance texts (e.g.
   *   Brealey/Myers/Allen, "Principles of Corporate Finance"). Banks/financial institutions are
   *   excluded - Graham himself (Benjamin Graham, "The Intelligent Investor") treats financial
   *   institutions' balance sheets as structurally not comparable to industrials on this ratio
   *   (deposits ARE liabilities by design), the same real-world reasoning the doc's own
   *   parameter table notes with "(Kecuali Bank)".
   * - PER x PBV > 22.5: Graham's own combined valuation ceiling (Benjamin Graham's classic
   *   rule PER<=15 AND PBV<=1.5, multiplied out to PER x PBV<=22.5) - the exact same constant
   *   already used for this codebase's own Graham Number fair-value formula
   *   (computeInvestmentLevels above, sqrt(22.5 x EPS x BVPS)). A stock failing this is, by
   *   the identical math, already trading above its own Graham fair-value ceiling - not a
   *   separately-invented number, the same one used elsewhere in this file.
   * - EPS_Growth_YoY < 0 for 2 consecutive quarters: the reference document's OWN literal
   *   Investment Stop Loss wording ("Spesifikasi Algorithmic Trading & ML.pdf" §2), checked
   *   here against two real consecutive quarters (see prev_eps_growth_yoy above), not just one.
   * - PEG > 2: Peter Lynch's own PEG methodology (Peter Lynch, "One Up On Wall Street", 1989) -
   *   PEG=1 is Lynch's fair-value baseline, PEG>2 is the commonly-cited secondary threshold in
   *   practitioner extensions of his framework for "clearly overvalued relative to its own real
   *   growth". (PEG<1 already gates the bullish side, straight from the same source.)
   * - Dividend Yield: deliberately NO bearish mirror. Graham's defensive-investor criteria
   *   treat an uninterrupted dividend record as a QUALITY marker, never a hard bearish gate on
   *   its own - a low/zero yield is completely normal for a legitimate growth company. The
   *   real, literature-documented bearish dividend signal is a CUT (not a low level), which
   *   needs a tracked quarterly dividend history to detect - data this pipeline doesn't
   *   currently carry, so it's honestly left out rather than approximated by yield level alone.
   */
  function investmentSignals(c) {
    const f = c.fundamentals || {};
    const hist = c.fundamentals_historical;
    const isBank = (c.sector || "").toLowerCase().includes("bank");
    let bull = 0, bear = 0;
    if (f.roe != null) { if (f.roe >= 15) bull++; else if (f.roe < 10) bear++; }
    if (!isBank && f.debt_to_equity != null) { if (f.debt_to_equity <= 100) bull++; else if (f.debt_to_equity > 200) bear++; }
    if (f.pe_ttm != null && f.pb_ratio != null && f.pe_ttm > 0 && f.pb_ratio > 0) {
      if (f.pe_ttm < 15 && f.pb_ratio < 1.5) bull++;
      else if (f.pe_ttm * f.pb_ratio > 22.5) bear++;
    }
    if (hist && hist.eps_growth_yoy != null) {
      if (hist.eps_growth_yoy >= 10) bull++;
      else if (hist.eps_growth_yoy < 0 && hist.prev_eps_growth_yoy != null && hist.prev_eps_growth_yoy < 0) bear++;
    }
    if (f.dividend_yield != null && f.dividend_yield >= 5) bull++; // bullish-only by design - see the note above
    if (hist && hist.peg != null && hist.peg > 0) { if (hist.peg <= 1) bull++; else if (hist.peg > 2) bear++; }
    return { bull, bear };
  }
  ...
  function buildStrategyList(strategy) {
    ...
    // Varian B (see the block above investmentSignals) - only for Swing/Investment, computed
    // once per strategy here rather than inside the per-ticker map for clarity.
    const verdictBFor =
      strategy === "swing"
        ? (c) => { const { bull, bear } = swingScore(c); return verdictFromCounts(bull, bear); }
        : strategy === "investment"
        ? (c) => { const { bull, bear } = investmentSignals(c); return verdictFromCounts(bull, bear); }
        : null;
