...
import { computeIndicators, resampleByWeek } from "./indicators.mjs";
...
/**
 * Weekly candles for the last ~6 months of trading (~130 sessions -> ~26 real calendar-
 * week bars) - used for the IHSG chart. More bars than a 1-month daily view, packed tighter,
 * so a longer-horizon pattern is actually visible instead of just recent noise. Refreshes
 * every day (this week's bar keeps extending until the week closes).
 */
async function fetchIhsgWeeklyCandles(symbol, days = 130) {
  const bars = await withRetry(() => zapi.chart({ symbol, count: days, resolution: "1D" }));
  return resampleByWeek(bars).candles;
}

export async function buildDashboard() {
  // ---- 1. Bulk data (Zapi Pro tier: cover the whole exchange, not just a slice) ----
  const [screenerItems, ffPage0, ffPage1, ffPage2, ffPage3, ihsgIndex, ihsgWeekly] = await Promise.all([
    ...
    fetchIhsgWeeklyCandles("IDX:COMPOSITE"),
  ]);
