...
export async function buildDashboard({ fundamentalsHistory } = {}) {
  ...
  /**
   * The Investment module's own fundamental gates, straight from Buku Putih §4A, as real
   * boolean bull/bear pairs (the reference table itself only defines the bullish/"attractive"
   * side of each metric - bearish thresholds are a reasonable, explicitly-approximate mirror
   * of that same real data, roughly matching the real deterioration levels the document's own
   * Investment stop-loss rule uses where it gives one, e.g. DER>2.0 and EPS_Growth_YoY<0).
   */
  function investmentSignals(c) {
    ...
    let bull = 0, bear = 0;
    if (f.roe != null) { if (f.roe >= 15) bull++; else if (f.roe < 5) bear++; }
    if (!isBank && f.debt_to_equity != null) { if (f.debt_to_equity <= 100) bull++; else if (f.debt_to_equity > 200) bear++; }
    if (f.pe_ttm != null && f.pb_ratio != null) {
      if (f.pe_ttm > 0 && f.pe_ttm < 15 && f.pb_ratio > 0 && f.pb_ratio < 1.5) bull++;
      else if (f.pe_ttm > 25 || f.pb_ratio > 3) bear++;
    }
    if (hist && hist.eps_growth_yoy != null) { if (hist.eps_growth_yoy >= 10) bull++; else if (hist.eps_growth_yoy < 0) bear++; }
    if (f.dividend_yield != null && f.dividend_yield >= 5) bull++; // doc only ever treats this as a positive attractor, no bearish mirror given
