...
export async function buildDashboard() {
  ...
  // ---- 5. Top 10 most active brokers (real IDX data via Zapi), aggregated across today's
  // top 5 net-buy picks rather than just one stock - so each broker's own highest-conviction
  // pick (among our shortlist) can be shown, not an unrelated ticker's avg price.
  // NOTE: IDX/Zapi only exposes combined (buy+sell together) per-broker activity for a
  // stock's session, not a true buy-side vs sell-side split - so "most active" ranks by
  // transaction value, not "net buyer". A single failed stock's broker summary is skipped,
  // never allowed to blank out the whole section.
  let activeBrokers = null;
  try {
    const topPicks = buyPicks.slice(0, 5);
    const perStock = await Promise.all(
      topPicks.map((s) =>
        withRetry(() => zapi.brokerSummary({ symbol: s.ticker, length: 200 }))
          .then((rows) => ({ ticker: s.ticker, rows }))
          .catch((e) => {
            console.error(`brokerSummary(${s.ticker}) unavailable, skipping this stock:`, e.message);
            return null;
          })
      )
    );
    // Sanity check: if the API is silently ignoring which stock we asked for (as it was
    // proven to do before the "code" param fix below), two different tickers come back with
    // byte-identical total transaction value - impossible for two real, different stocks.
    // Log it loudly so a future regression here is obvious in the Actions log, not silently
    // wrong again.
    const valueSignatures = perStock.filter(Boolean).map((r) => ({ ticker: r.ticker, total: r.rows.reduce((a, row) => a + (row.Value || 0), 0) }));
    for (let i = 0; i < valueSignatures.length; i++) {
      for (let j = i + 1; j < valueSignatures.length; j++) {
        if (valueSignatures[i].total > 0 && valueSignatures[i].total === valueSignatures[j].total) {
          console.error(`brokerSummary sanity check FAILED: ${valueSignatures[i].ticker} and ${valueSignatures[j].ticker} returned identical total value (${valueSignatures[i].total}) - the API is likely still ignoring which stock was requested.`);
        }
      }
    }
    ...
    let latestDate = null;
    for (const stockResult of perStock) {
      if (!stockResult || !stockResult.rows.length) continue;
      // brokerSummary({length:200}) returns up to 200 rows that can span multiple past
      // sessions, not just today's - mixing those in would silently average/compare prices
      // across different trading days for the same stock (e.g. right-after-IPO vs weeks
      // later), which is exactly what produced wildly inconsistent "avg price" per broker
      // for the same top-pick ticker. Keep only this stock's own most recent session.
      const stockLatestDate = stockResult.rows.reduce((max, r) => {
        const d = r.Date ? r.Date.slice(0, 10) : null;
        return d && (!max || d > max) ? d : max;
      }, null);
      const todaysRows = stockResult.rows.filter((r) => (r.Date ? r.Date.slice(0, 10) : null) === stockLatestDate);
      if (stockLatestDate && (!latestDate || stockLatestDate > latestDate)) latestDate = stockLatestDate;
      for (const row of todaysRows) {
        const avgPrice = row.Volume ? Math.round(row.Value / row.Volume) : null;
        const agg = brokerMap.get(row.IDFirm) || { broker: row.IDFirm, name: row.FirmName, volume: 0, value: 0, frequency: 0, topPick: null };
        agg.volume += row.Volume;
        agg.value += row.Value;
        agg.frequency += row.Frequency;
        // This broker's single highest-value stock among our top 5 picks - real per-stock
        // value/avg price, just picking the max rather than estimating a portfolio-wide one.
        if (!agg.topPick || row.Value > agg.topPick.value) agg.topPick = { ticker: stockResult.ticker, value: row.Value, avg_price: avgPrice };
        brokerMap.set(row.IDFirm, agg);
      }
    }
    const top10 = [...brokerMap.values()]
      ...
      .map((b) => ({
        ...
        top_pick_ticker: b.topPick.ticker,
        top_pick_avg_price: b.topPick.avg_price,
      }));
    activeBrokers = { tickers: perStock.filter(Boolean).map((r) => r.ticker), date: latestDate, top10 };
  } catch (e) {
  ...
  // "trading_date" is the date of the actual market session this data is from - taken from
  // the real last IHSG daily candle, NOT from whatever day the script happens to run. This
  // matters because the pipeline now also runs Sunday evening (a deliberate "Monday prep"
  // run on Friday's real closing data): trading_date correctly still says Friday's date,
  // it's never mislabeled as "Sunday's data" just because that's when the job executed.
  const lastSessionCandle = ihsg.candles[ihsg.candles.length - 1];
  const tradingDate = lastSessionCandle ? wibDateOf(lastSessionCandle.t) : wibNow.toISOString().slice(0, 10);
