...
export async function buildDashboard() {
  ...
  // "Success Rate" - a transparent, bounded (50-90%) heuristic estimate of how much real
  // room a Buy/Strong Buy call still has to rise FROM the recommended entry price, built
  // ONLY from real signals already computed above (never a separate guess). It is explicitly
  // NOT a backtested probability - just how strongly this stock clears its strategy's own
  // bullish criteria right now, adjusted down when the stock already looks extended (RSI deep
  // in overbought territory means less real upside is likely left before a pullback, which
  // matters directly for "will it still rise from here"). Shown to the user labeled as an
  // estimate, never as a guaranteed number.
  function potentialPct(strategy, c, verdict) {
    ...
    if (strategy === "scalping") {
      ...
      // Scalping explicitly chases breakout momentum, so overbought RSI(5) is the expected,
      // healthy state of a live breakout - not penalized here the way it is for the
      // slower strategies below.
      ...
      // Buying deep into overbought leaves less real room to rise before a pullback -
      // directly relevant to "will this still go up from the entry price".
      if (c.rsi14 != null && c.rsi14 > 78) score -= 6;
    } else {
      if ((c.pattern || "").includes("uptrend sehat")) score += 12;
      if (c.rsi14 != null && c.rsi14 > 50 && c.rsi14 < 65) score += 10;
      if (c.sma20 != null && c.sma50 != null && c.sma20 > c.sma50) {
        score += 8;
        // Trend strength: how far SMA20 has pulled away from SMA50 - a young, still-widening
        // spread means the move is more likely still ahead rather than already priced in.
        score += Math.max(0, Math.min(8, ((c.sma20 - c.sma50) / c.sma50) * 100));
      }
      if (c.prior50High != null && c.lastClose > c.prior50High) score += 8;
      if (c.rsi14 != null && c.rsi14 > 75) score -= 6;
    }
