...
function computeInvestmentLevels(verdict, c, hasUpcomingCorporateAction) {
  ...
  // REAL figures reported in that company's own IDX XBRL filing (net_income/shares,
  // total_equity/shares - scrapers/idx-fundamentals/, see fundamentals_historical above) when
  // that ticker's been scraped; otherwise derived from the real PER/PBV we always have
  ...
  let fairValue = null;
  if (hist && hist.eps > 0 && hist.bvps > 0) {
...
export async function buildDashboard({ fundamentalsHistory } = {}) {
  // Real per-ticker EPS/BVPS/growth from actual IDX XBRL filings (scrapers/idx-fundamentals/,
  // run occasionally via .github/workflows/fundamental-scraper.yml - never fetched live here).
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      // Real EPS/BVPS/growth from actual IDX XBRL filings, when this ticker's been scraped -
      ...
      // fabricated value.
      const fundamentals_historical = fundamentalsHistoryByTicker[s.ticker]?.latest || null;
