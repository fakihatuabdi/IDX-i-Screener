...
/**
 * Daily candles for the last ~month of trading (~22 sessions) - used for the IHSG chart,
 * long enough for a real pattern (not just noise) to be visible. Refreshes every day.
 * Per-stock cards reuse candles already fetched for indicators instead of calling this.
 */
async function fetchDailyCandles(symbol, days = 22) {
...
export async function buildDashboard() {
  ...
  const [screenerItems, ffPage0, ffPage1, ffPage2, ffPage3, ihsgIndex, ihsgDaily] = await Promise.all([
    ...
    fetchDailyCandles("IDX:COMPOSITE"),
  ]);
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      // Daily candles for the chart shown on the card - the last ~month, refreshed each
      // day. No extra API call needed: computeIndicators() already returns the last 20
      // daily bars from the 210-day history we just fetched for the indicators above.
      const dailyCandles = ind.candles20;
      ...
      return { ...s, ...ind, tv_verdict, cap_tier: capTier(s.marketCap), dailyCandles, fundamentals };
    } catch (e) {
  ...
  // oversold) zone, is the bullish case. Mirrors on the bearish side with MA20 below MA50,
  // a Lower-High/Lower-Low structure, and price below MA20.
  function swingVerdict(c) {
    ...
    if (uptrend && healthyPattern) return "Strong Buy";
    if (uptrend || (healthyPattern && rsiConstructive)) return "Buy";
    if (downtrend && downPattern) return "Strong Sell";
    if (downtrend) return "Sell";
    return "Hold";
  }

  // Investment: long horizon. Uses TradingView's own real technical rating (the one real
  // source with a genuine 5-tier scale) rather than our own short-horizon indicators - a
  // blue chip with a real Sell/Neutral rating is never forced into Buy just for being a
  // blue chip. Falls back to the SMA-derived verdict only if technicals() itself failed.
  ...
  function buildStrategyList(strategy) {
    ...
    const technicalFor = (c) =>
      strategy === "scalping"
        ? `EMA13 ${c.ema13} | EMA21 ${c.ema21} | RVOL ${c.rvol}x | RSI(5) ${c.rsi5} | Prior20High ${c.prior20High}`
        : strategy === "investment"
        ? `SMA50 ${c.sma50} | SMA200 ${c.sma200} | Rating TradingView: ${c.tv_verdict || "N/A"}`
        : `SMA20 ${c.sma20} | SMA50 ${c.sma50} | RSI(14) ${c.rsi14} | Pola: ${c.pattern}`;
    ...
    const items = withIndicators.map((c) => {
      ...
      return {
        ...
        pattern: c.pattern,
        rvol: c.rvol,
        candles: c.dailyCandles,
        fundamentals: c.fundamentals,
        _sortStrength: strengthScore(verdict),
        _sortFundamental: strategy === "investment" ? fundamentalScore(c.fundamentals) : 0,
        // Real daily-timeframe MA/EMA series (already computed above, no extra cost),
        // aligned day-for-day with the chart's own candles - drawn as a real line that
        // follows price shape over time, not a flat reference line at today's value.
        // Which line pair is relevant depends on strategy: EMA13/21 for Scalping, SMA20/50
        // for Swing, SMA50/200 for Investment.
        ma_lines: { sma20: c.sma20Series, sma50: c.sma50Series, sma200: c.sma200Series, ema13: c.ema13Series, ema21: c.ema21Series },
      };
    });
