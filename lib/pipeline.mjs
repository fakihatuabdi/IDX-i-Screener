...
// close: each strategy anchors its entry zone to the support/resistance level that matches
// its own timeframe - Scalping reads EMA9 (its fastest support), Swing reads SMA20,
// Investment reads SMA50 - the same anchors each strategy's own indicator set already trades
// off of (see technicalFor below). The anchor is only used when it's realistically reachable:
// bounded by that stock's own ATR(14), so a deep, unlikely-to-fill support never gets used
// as-is, and a Buy entry zone never sits above today's real close (you can't "wait for a
// pullback" above the current price). Stop-loss is sized off that same real ATR risk, then
// nudged just past a genuine nearby swing low/high (matched to the strategy's horizon) when
// that real level sits tighter than the ATR stop - otherwise ordinary noise at that level
// would trigger the stop before the level itself is even tested. Target is a reward:risk
// multiple of that same real risk distance, never an arbitrary fixed percentage.
const RISK_CONFIG = {
  scalping: { supportKey: "ema9", stopAtrMult: 1.0, rr: 1.5, maxAtrMult: 0.6, narrowAtrMult: 0.2, structKey: "prior20" },
  swing: { supportKey: "sma20", stopAtrMult: 1.5, rr: 2.0, maxAtrMult: 1.2, narrowAtrMult: 0.4, structKey: "prior50" },
  investment: { supportKey: "sma50", stopAtrMult: 2.0, rr: 2.5, maxAtrMult: 2.5, narrowAtrMult: 0.8, structKey: "year" },
};
...
function computePriceLevels(strategy, verdict, c) {
  ...
  if (verdict === "Hold") {
    ...
    return { entry_low: null, entry_high: null, target: Math.round(lastClose + band), stop_loss: Math.round(lastClose - band) };
  }
  ...
  if (verdict === "Sell" || verdict === "Strong Sell") {
    ...
    // how far a real ATR-sized stop and reward:risk target sit from that real price.
    ...
    const target = lastClose - cfg.rr * risk;
    return { entry_low: null, entry_high: null, target: Math.round(target), stop_loss: Math.round(stopLoss) };
  }

  // Buy/Strong Buy: a real accumulation ZONE, not one exact price - from a genuine nearby
  // support level (matched to this strategy's own timeframe) up to today's actual close,
  // bounded by this stock's own ATR so the low end is never a support too deep to realistically
  // fill. When no usable support sits nearby, fall back to a narrow zone just under today's
  // close (buying confirmed strength, not waiting for a pullback that isn't there).
  ...
  // range; target is projected conservatively from the top of the zone (the worst-case real
  // entry price within it) so it stays a valid, honest projection no matter where the order
  ...
  const target = entryHigh + cfg.rr * risk;
