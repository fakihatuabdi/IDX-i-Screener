...
/**
 * Daily candles for the last trading week (~5 sessions) - used for the IHSG chart.
 * Refreshes every day (today's close becomes a new bar, the oldest one rolls off).
 * Per-stock cards reuse candles already fetched for indicators instead of calling this.
 */
async function fetchDailyCandles(symbol, days = 7) {
...
export async function buildDashboard() {
  ...
  async function fetchIndicatorsFor(s) {
    try {
      ...
      // Daily candles for the chart shown on the card - the last trading week, refreshed
      // each day. No extra API call needed: computeIndicators() already returns the last
      // 20 daily bars from the 210-day history we just fetched for the indicators above.
      const dailyCandles = ind.candles20.slice(-7);
  ...
  function buildStrategyList(strategy) {
    ...
    const items = withIndicators.map((c) => {
      ...
      return {
        ...
        // drawn as reference lines on the weekly chart so the pattern can be read against
        // the stock's actual trend context (which line set is relevant depends on strategy:
        // EMA13/21 for Scalping, SMA20/50 for Swing, SMA50/200 for Investment).
