...
export async function buildDashboard() {
  ...
  // inverting the lookup. Scanning all ~880 listed stocks isn't practical call-budget-wise,
  // so this scans the day's most actively-traded stocks (already sorted by volume from the
  // screener call) - a big broker's single biggest transaction is overwhelmingly likely to be
  // in one of the market's most liquid names, not an illiquid one. A broker whose real top
  // pick fell outside this scan window simply gets no pick shown - never guessed.
  const MOST_ACTIVE_SCAN_COUNT = 40;
