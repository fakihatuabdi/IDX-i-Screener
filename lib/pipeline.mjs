...
export async function buildDashboard() {
  ...
  // dimension at all (a real ticker, an invalid one, and no ticker param all returned
  // byte-identical data) - there is no way to get "which stock did broker X trade most" from
  // this data, so this only ranks brokers by their total activity across the whole exchange.
  // Combined (buy+sell together) - IDX/Zapi doesn't expose a buy vs sell split here, so this
  // ranks brokers by how active they were, not classifies them as net buyers/sellers.
